################################################################################
##    Augment data with by sampling multiple average values of the latent     ##
##                          process for each interval                         ##
################################################################################

# This file augments each bootstrapped subset with new synthetic values of the
# average latent process over each event interval (but does not bootstrap).

################################################################################
# Step 1: Generate R synthetic values of the average of the latent process over
#  each event interval for this dataset
################################################################################
...
all_etas <- matrix(nrow = 0, ncol = 7,
                   dimnames = list(NULL,
                                   c('ibs', 'm', 'r', 'start_time', 'stop_time',
                                     'avg_eta1', 'avg_eta2')))
...
for (ibs in 1:N){ # ibs = unique user id for bootstrapping
  ...
  all_etas <- rbind(all_etas, cbind(ibs, avg_etas_i))
}
